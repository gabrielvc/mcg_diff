...
class Inpainting(H_functions):
    def __init__(self, channels, img_dim, missing_indices, device):
        ...
        self._singulars = torch.nn.Parameter(torch.ones(channels * img_dim ** 2 - missing_indices.shape[0]).to(device),
                                             requires_grad=False)
        self.missing_indices = torch.nn.Parameter(missing_indices, requires_grad=False)
        self.kept_indices = torch.nn.Parameter(
            torch.Tensor([i for i in range(channels * img_dim ** 2) if i not in missing_indices]).to(device).long(),
            requires_grad=False)
...
class Deblurring(H_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, ZERO=3e-2):
        ...
        self._singulars = torch.nn.Parameter(
            torch.matmul(self.singulars_small.reshape(img_dim, 1), self.singulars_small.reshape(1, img_dim)).reshape(
                img_dim ** 2))
...
class Deblurring2D(H_functions):
    ...
    def __init__(self, kernel1, kernel2, channels, img_dim, device):
        ...
        self.U_small1, self.U_small2 = torch.nn.Parameter(self.U_small1, requires_grad=False), torch.nn.Parameter(
            self.U_small2, requires_grad=False)
